class FS {
  ...
  async upload(blob) {
    let res = await this.core.endpoint.request("workspace", "POST", "/fs/binary", {
  ...
  async json(o) {
    let res = await this.core.endpoint.request("workspace", "POST", "/fs/json", {
      workspace: this.core.o.workspace,
      body: o,
    })
    return res.cid
  ...
  async rm(cids) {
    await this.core.endpoint.request("workspace", "POST", "/fs/rm", {
      workspace: this.core.o.workspace,
      cids: cids
    })
  }
  async pin(cid, cb) {
    ...
    if (cb) {
      interval = setInterval(async () => {
        let r = await this.core.endpoint.request("workspace", "POST", "/fs/pin/progress", {
          workspace: this.core.o.workspace
        })
        cb({
    ...
    let res = await this.core.endpoint.request("workspace", "POST", "/fs/pin", {
      workspace: this.core.o.workspace,
      cid
    })
